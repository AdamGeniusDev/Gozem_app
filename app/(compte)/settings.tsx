  ...
  return (
    <SafeAreaView className=' bg-white flex-1'>
      <View className='px-5'>
        ...
        <Text className='font-semibold text-[16px]'>Parametres</Text>
      </View>

      <View className='bg-white' style={{marginTop: 25, paddingBottom: 25}}>
        {mainSettings.map((item, i) => (
          <Pressable key={item.title} onPress={item.onPress} className='flex-row gap-x-5 items-center' style={{height: 70}}>
            <Image source={item.icone} style={{height: 25,width:25}} resizeMode='contain' tintColor={'#B0B3B2'} />
            <View className='flex-row justify-between items-center flex-1 border-neutral-100' style={{borderBottomWidth: i=== (mainSettings.length -1)? 0 : 1, height: 70}}>
      ...
      <View className='bg-neutral-100' style={{height: 90}}>
        <Pressable className='bg-white mt-3 px-5 flex-row gap-x-3 items-center rounded-lg' style={{height: 70}} onPress={()=> modalRef.current?.present()}>
          <Image source={images.logout} style={{height: 30,width:30}} resizeMode='contain' tintColor={'#B0B3B2'} />
          <Text className='font-regular text-[16px]'>Deconnexion</Text>
        </Pressable>
      </View>

      <Pressable className='flex-row px-5 gap-x-4 items-center' style={{paddingTop: 20}} hitSlop={12} onPress={()=> router.push('/(settings)/deleteAccount')}>
          <Image source={images.deleted} style={{height: 25,width:25}} resizeMode='contain' tintColor={'#B0B3B2'}/>
          <Text className='text-[16px] font-regular'>
            Supprimer le compte
          </Text>
      </Pressable>
      ...
      >
        <BottomSheetView className='flex-1 px-5 py-5'>
          <View className='justify-center items-center flex-1 gap-y-3 flex-col'>
            <Text className='font-poppins-bold text-center text-[20px]'>Deconnexion</Text>
            <Text className='font-roboto text-[13px] text-center text-neutral-700 mb-3'>Etes vous sur de vouloir vous deconnecter? Vous devrez saisir a nouveau vos informations d'identification lorsque vous souhaiter vous connecter</Text>
            {
              loading? (<CustomButton titre='Deconnexion...' disabled={true} />):<CustomButton titre='Oui, deconnexion' disabled={false} onPress={logoutUser} />
            }
