...
const Zem = () => {
  ...
  const tab = [
    t('services.orderFor.options.gozemContact'),
    t('services.orderFor.options.phoneContact'),
    t('services.orderFor.options.someoneElse')
  ]
    ...
    <View className='px-4 pt-2'>
      <View className='flex-row max-w-full justify-between'>
        <Text
          ...
          style={{ width: '50%' }}>
          {t('services.headers.definePickup')}
        </Text>
        <Pressable 
          ...
          <View className='flex items-end'>
            <Text className='font-regular text-neutral-500 text-[12px]'>{t('services.orderFor.label')}</Text>
            <Text className='font-medium text-[12px]'>{t('services.orderFor.me')}</Text>
          </View>
          <View>
      ...
      <View className='flex-row py-3 gap-x-3 items-center'>
        ...
        <View className='flex-1 flex gap-y-3'>
          <Pressable className='bg-neutral-100 flex-row items-center rounded-lg border-neutral-200 border-[1px] w-full p-2 justify-between' style={{height: 55}}>
            <View>
              <Text className='font-regular text-neutral-500 text-[12px]'>{t('services.addresses.pickupLabel')}</Text>
              <Text className='font-medium text-[14px]'>{t('services.addresses.pickupCurrent')}</Text>
            </View>
            <Image source={images.depart} className='w-10 h-10' resizeMode='contain'/>
          </Pressable>
          <Pressable className='bg-neutral-100 flex-row items-center rounded-lg border-neutral-200 border-[1px] w-full p-2 justify-between' style={{height: 55}}>
            <Text className='font-regular text-neutral-500 text-[14px]'>
              {t('services.addresses.dropoffLabel')}
            </Text>
  ...
  const SecondRoute = () => (
     ...
     <View className='py-3 items-center px-3 flex-1 justify-center' style={{gap: 15}}>
      ...
      <Pressable className='bg-neutral-100 flex-row items-center h-[60px] rounded-lg border-neutral-200 border-[1px] w-full p-2 justify-between'style={{height: 55}}>
            <View>
              <Text className='font-regular text-neutral-500 text-[12px]'>{t('services.addresses.pickupLabel')}</Text>
              <Text className='font-medium text-[14px]'>{t('services.orderFor.me')}</Text>
            </View>
            <Image source={images.depart} className='w-10 h-10' resizeMode='contain'/>
        </Pressable>
      </View>
      ...
      <Pressable className='bg-neutral-100 flex-row items-center h-[60px] rounded-lg border-neutral-200 border-[1px] w-full p-2 justify-between' style={{height: 55}}>
            <View>
              <Text className='font-regular text-neutral-400 text-[14px]'>{t('services.duration.label')}</Text>
            </View>
  ...
  const routes = [
    {
      key: 'first', title: t('services.tabs.byDistance'), component: FirstRoute
    },
    {
      key: 'second', title: t('services.tabs.byDuration'), component: SecondRoute
    }
  ]
